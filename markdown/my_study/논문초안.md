...
- 4. Ablation Study (선택 사항)
    - PI, GAF, Signal 중 일부 Teacher를 제거했을 때 성능이 어떻게 변하는지 추가 실험을 통해 검증.
    - Logits-based KD만 적용했을 때, Feature-based KD만 적용했을 때의 비교 실험을 수행.
    - Annealing 기법 적용 여부에 따른 성능 차이를 분석.
