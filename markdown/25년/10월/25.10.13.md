...
- Trainer에서 정의해야 하는 것들 : Epoch, devices(GPU/CPU), acclerator(cpu/gpu/tpu), precision, logger, callbacks, log_every_n_steps(로그 출력 주기), check_val_every_n_epoch(몇 epoch마다 val을 수행할지), gradient_clip_val(gradient clipping용), limit_train_batches(디버깅용 batch 수 제한), enabel_progress_bar(tqdm progress bar 출력 여부) 의 '훈련을 어떻게 수행할지'를 정하는 역할. 훈련 환경, 장치, epoch수, 로깅 주기 등을 설정함.
