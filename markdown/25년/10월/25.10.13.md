...
- Trainer에서 정의해야 하는 것들 : '훈련을 어떻게 수행할지'를 정하는 역할. 훈련 환경, 장치, epoch수, 로깅 주기 등을 설정함.
    - Epoch
    - devices(GPU/CPU)
    - acclerator(cpu/gpu/tpu)
    - precision
    - logger
    - callbacks
    - log_every_n_steps(로그 출력 주기)
    - check_val_every_n_epoch(몇 epoch마다 val을 수행할지)
    - gradient_clip_val(gradient clipping용)
    - limit_train_batches(디버깅용 batch 수 제한)
    - enabel_progress_bar(tqdm progress bar 출력 여부)
...
class LitVQShape(L.LightningModule):
    def __init__(self, args):
      ...
      self.save_hyperparameters(args) # 이것 때문에 아레에서 self.hparams 로 저 args에 접근이 가능한것임.
      try:
        self.model = VQShape( # 이거는 VQShape model을 생성할 때 필요한 생성자들 hyperparameter들.
            len_input=self.hparams.normalize_length, # 정규화된 길이
            dim_embedding=self.hparams.dim_embedding, # embedding 크기
            patch_size=self.hparams.patch_size, # patch 크기 (time-series를 여러 개의 patch로 나눴을 때 각 patch의 길이)
            num_patch=self.hparams.num_patch, # patch 개수 
            num_token=self.hparams.num_token, # token 개수
        ...
        self.model = VQShape(
            ...
            num_code=self.hparams.num_code,
            dim_code=self.hparams.dim_code,
            codebook_type='standard', # 이게 standard인게 다른듯
            lambda_commit=self.hparams.lambda_vq_commit,
            lambda_entropy=self.hparams.lambda_vq_entropy,
            entropy_gamma=self.hparams.entropy_gamma,
            mask_ratio=self.hparams.mask_ratio
    ...
    def training_step(self, batch, batch_idx): # 학습을 어떻게 할지에 대한 정의.
      _, loss_dict = self.model(batch, mode='pretrain') # 모델의 mode는 pretrain이고, pretrain의 반환은 output_dict, loss_dict임.
      ...
      # log에 남기기 위해서 loss_dict를 만들고, 이 dictionary를 이용해서 우리한테 시각적으로 보여줌.
      loss_dict = {
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0): # model.val()쪽 전체를 대신하는 클래스 메서드
        output_dict, loss_dict = self.model(batch, mode='evaluate')

        # 최종 loss 계산을 함. (hyperparameter로 정한 각 loss 비중 포함)
        loss = self.hparams.lambda_x * loss_dict['ts_loss'].mean() +\
            self.hparams.lambda_z * loss_dict['vq_loss'].mean() + \
            self.hparams.lambda_s * loss_dict['shape_loss'].mean() + \
            self.hparams.lambda_dist * loss_dict['dist_loss'].mean()
        
        # 최종 loss와 각 loss 구성요소의 평균 loss까지 포함해서 loss_dict를 다시 구성
        loss_dict = {
        ...
        # 다 포함된 loss_dict를 이용해서 log_dict를 만듦. 로그를 뿌리기 위함.
        self.log_dict(loss_dict, sync_dist=True)
        
        # Validation Set에서 Model이 예측한 z와, t, l들을 저장함.
        self.validation_step_outputs[dataloader_idx]['z'].append(output_dict['code_idx']) # code_idx는 이번에 들어온 Time-series가 Codebook의 어떤 코드에 매핑되었는지를 의미하는 Index
        self.validation_step_outputs[dataloader_idx]['t'].append(output_dict['t_pred']) # t, l은 Attribute Decoder가 예측한 값 인것 같음.
    ...
    def on_validation_epoch_end(self): # 각 epoch이 끝난 후 할 것.
        # 모델이 모든 code를 균형있게 사용하고 있는가 : z_idx로 판단
        # t, l도 고루게 얻어져야 하기 때문에 t, l도 함께 그려본 듯.
