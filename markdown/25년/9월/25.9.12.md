...
### 그래서 CCT가 어떻게 동작하는지
- <img src="https://github.com/wjdwocks/ML-DNN/raw/main/markdown/25년/9월/25.9.12/CCT.png" alt="results" width="700">
...
- 목적 함수
    ...
    - 목적 함수는 이것이고, x는 CLIP Image Embedding이고, Ec는 Clip Text Embedding임.
...
- 어떻게 동작하는지에 대한 간단한 overview?
    ...
    - 또한, 그 둘을 dot product 하면, modality가 달라도, 같은 Clip으로 얻은 embedding vector이기 때문에, similarity를 계산할 수 있다.
    - 이를 통해서 Concept Slot은 Text로 구성하지만, Input이 Text가 아니더라도, 사용이 가능하다. 
...
2. CLIP을 이용한 Submodular Concept Selection:
    ...
    - 이렇게 얻어진 k개의 concept subset을 학습에 사용한다.
    - 또한, 이 submodular에 의한 subset을 얻는 과정은 학습되는 부분이 아님(그냥 다 수식계산으로 이루어짐.)
3. 어떻게 class 예측을 하는가
    - <img src="https://github.com/wjdwocks/ML-DNN/raw/main/markdown/25년/9월/25.9.12/LaBo_predict.png" alt="results" width="700">
    1. 위에서 선별된 concepts들을 이용해서 Concept Space를 얻는다.
        - 클래스 개수 N, concept 개수 k라고 했을 때 (N x k, d)의 shape이 나옴. (d는 vector 길이)
        - 이 때 사용하는 Text Encoder도 Clip으로 사용(학습 안함.)
    2. input image를 concept space에 대한 표현으로 변환. (어쩌면 CA랑 비슷할지도?)
        - input image를 다시 Clip으로 embedding vector로 변환함.
        - 이 vector와 Concept Space의 모든 vector와 dot product를 해서, 각 concept과 input image와의 concept score를 계산함.
        - 결과로, (1, N x k)의 score vector가 생성됨.
    3. 이 concept score를 이용하여 최종 class 예측을 수행.
        - Class-Concept Weight Matrix(trainable, (N, N x k))라는 가중치 행렬과 score vector의 transpose를 곱해주면 class distribution probability가 계산됨. 
        - 아마도, (1, N x k)의 score vector를 입력으로 받는 classifier network에 넣어서 최종 클래스 예측을 수행한다는 의미인듯.
...
- 하지만, 서로 다른 modality로 얻어진 features를 direct로 합치는 것은 heterogeneous dimension size를 갖고, 두 정보의 의미적인 alignment에 어려움이 있을 수 있다.
- 그래서, mGLW 모듈을 도입한다.
...
- 각 Network의 마지막 Block의 output Feature들의 Shape을 맞춰주고(Embedding input, e) Teacher들의 정보(eT1, eT2)로만 Knowledge Slot을 업데이트함. → CCT의 CSA모듈과 동일
- Teacher들의 정보로 업데이트된 Knowledge Slot을 이용해서, Student의 정보(eS)를 Knowledge Slot에 대한 표현으로 변환한 후, 최종 클래스 예측에 사용. → CCT의 CA모듈과 동일

### LaBo
- Image에 대해서 수행함.
- 데이터셋에 대해서, Concept Slot을 LLM과 submodular 계산을 이용하여 자동으로 얻는다. → Slot 생성 부분
- 그렇게 얻어진 Concept Slot을 이용해서, Input Image를 Concept Slot에 대한 표현을 Classifier에 넣어 최종 출력을 받는 다는 점에서 CA와 거의 동일함.
- 하지만, Input Image를 이용해서 Concept Space를 업데이트하지는 않는다는 점에서 CCT의 CSA와 좀 다른듯 하다.



## Language Guided Concept Bottleneck Models for Interpretable Continual Learning 읽어보기.
### Continual Learning이 무엇인가?
- CL은 모델이 여러 Task / Dataset을 순차적으로 학습하면서, 과거에 배운 지식을 잊지 않고 누적해 나가도록 하는 학습 패러다임.
- 현실 환경에서 그러하듯, 데이터 분포와 과제가 시간에 따라 달라지는 것에 대해서 재학습 비용을 줄이고, 여러 데이터에 대한 적응성을 높이려는 목적임.
- 즉, 새 지식을 지속적으로 습득하면서도, 과거 지식을 잊지 않게 하는 것이 CL의 목적임.
    - 그 중에서 이 논문은 CIL(Class Incremental Learning)에 초점을 두는데
    - 각 단계별로, 학습할 클래스가 추가되는 형태.
    - 1단계 : 개, 고양이를 학습.
    - 2단계 : 새, 물고기를 추가로 학습
    - → 이후, 모델은 개, 고양이, 새, 물고기를 모두 분류할 수 있어야 함.
- Catastrophic Forgetting
    - Continual Learning 중에 최신 data/task에 맞춰 파라미터가 업데이트될 때, 이전 data/task에서 유용했던 표현과 Decision boundary가 무너져 과거 성능이 급격히 약화되는 현상.
    - Continual Learning의 가장 큰 목적은 이 Catastrophic Forgetting을 줄이는 것임.

### Introduction
- In real world에서는, 환경이 지속적으로 변화함에 따라, 고정된 데이터로 학습된 모델이 새로운 task/data에 바로 사용되기에는, catastrophic forgetting 문제가 있다.
- 그래서 이것을 해결하기 위한 방법으로 Continual Learning이 있다.
- Continual learning은 모델이 이전의 지식도 보존하면서, 새롭게 얻어지거나 변화하는 task/data에도 잘 적응할 수 있도록 하는 학습 방식이다.

- 기존의 Continual Learning의 학습 방법은 catastrophic forgetting 문제를 완화하는데 초점을 맞추고 있지만, 의도치 않은 행동을 방지하기 위해서, 근본적으로 무엇을 학습하고, 이전 정보를 어떻게 유지하는지에 대한 이해가 중요하다. 
- 또한, CL 모델의 내부 메커니즘을 이해한다면, 모델 설계를 개선해서 더욱 높은 성능을 끌어낼 수 있음.

- interpretable 모델을 만드는 것 중 대표적인 것이 CBM이다. (바로 predict하지 않고, 중간에 concept bottleneck을 한번 거친 후 최종 예측에 사용.)
- 또한 이 CBM개념을 이용한 LaBo 프레임워크를 이용한다는 것인듯. (LLM을 이용해서 Concept Bottleneck을 자동으로 수집한다.)
