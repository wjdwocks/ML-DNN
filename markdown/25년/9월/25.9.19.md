...
1. 일단 model을 찾은듯
    ...
    def pretrain(self, x: torch.Tensor):
        ...
        x_embed = self.encoder.patch_and_embed((x - self.x_mean)/self.x_std) # 여기 encoder가 PatchEncoder임.
        batch_size, num_patch, patch_dim = x_embed.shape # Shape이 (B, num_patch, patch_dim인거로 봐서) Linear Encoder 들어가기 전인듯 함. == (B, 64, 256)
