...
### 몇개 결과
- 가중치는 UEA에서 사용한 것 말고, 전에 GENEActiv에서 Reconstruction만 했을 때 가장 잘 됐던거로 사용하였고, UEA에서 학습한 가중치 중 중복으로 사용이 되는 Transformer에만 불러옴.(근데 Tokenizer의 transformer만 불러와짐. 왜 Time-series Encoder/Decoder의 Transformer는 안되지?)
...
- 이상한 것은 3(Treadmill 4mph), 6(Seated-fold/stack laundry)번 클래스에 대해서는 예측이 이루어지지 않고 있음. → 이게 어떻게 해결이 되면 60%대까지 오를 수 있을지도.. 
- <img src="https://github.com/wjdwocks/ML-DNN/raw/main/markdown/25년/12월/25.12.26/prediction.png" alt="results" width="700">
...
- histogram(wrn hetero) legend 위치 가로로 + y-max 줄이기 (완)
- reference, processing time 표 수정 (완)
- Conclusion쪽에 noise처럼 되는 모달리티를 어떻게 개별 batch나 샘플에 적용할 수 있을지에 대한 future work같은 내용 (완)
- 이미지나 표 캡션들 대폭 수정 필요(넣거나 수정하거나)
