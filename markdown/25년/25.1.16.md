...
<li> ** Knowledge Distillation for Object Detection with Diffusion Model - Diffution</li>
<li> ** Foot Pressure-based Abnormal Gait Recognition with Multi-scale Cross-Attention Fusion - Cross Attention, Fig6를 완전히 이해해보다. (Attention 매커니즘 (Transformer 이해 필요.)) </li>
</ol>
...
<li> 논문에서 사용된 GAF이미지 (클래스별로 대표 이미지를 보여준듯 하다.)는 y = -x 대각선에 대칭인 것으로 보아 GASF를 사용한 것 같다. (딱히 언급이나, 이유는 적혀져 있지 않다.), (cos(θi + θj)나 cos(θj + θi)나 같기에 GASF는 대칭임.) </li>
