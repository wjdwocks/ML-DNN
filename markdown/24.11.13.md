...
<li> 교사 모델이 일반적으로 더 복잡하고, 섬세한 정보를 포함하고 있다. </li>
...
<li> AMD(g) </li>
<ul>
<li> 기본 아이디어 : 특성의 각도를 고려하여, 교사 모델과 유사한 각도를 가지도록 학생 모델을 학습한다. (Attention map에서 추출한 Feature Vector얘기하는것임.) </li>
<li> 방법 : 교사 모델의 타켓 클래스 각도에 Angular Margin을 추가하여, 각 클래스 간의 분포를 더 분리시켜 학생 모델이 그 분포를 따라하게 유도하는 것. </li>
<li> 즉, AMD에서의 손실은 기존의 KD(CrossEntropy항 + KL divergence항 + AM loss항이다.) </li>
<li> global의 의미 :  </li>
<li> loss 함수 : AM loss항은 A + P + N으로 나타낼 수 있는데, A항은 학생 모델이 교사 모델의 Angular Distance를  </li>
</ul>
<li> AMD(g+l) </li>
<ul>
<li>  </li>
