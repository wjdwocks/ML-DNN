...
5.  그러면서 3Channel 나누어서 학습하는것도 고려해보면 좋을듯. (부수적)
    ...
    - 3Channel에서 UEA와 같은 Hyperparameter로 수행했는데 생각보다 잘 안됨. → 왜일까?
        - 혹시 CE_loss가 0인 구간에서 Classifier를 Freeze해야하는것인가?
        - 코드를 검토해봤을 때 (Data Provider에서 Norm이 되었는지)
        - Flatten때와 비교해서 뭘 다르게 설정한게 있었는지?
        - Hyperparameter에 대한 변화가 필요한지? (Token수나 차원 등)를 봤는데 보이진 않았음.
